...
const CACHE_NAME = isDevelopment ? `liseng-dev-${Date.now()}` : 'liseng-v1';
const RUNTIME_CACHE = isDevelopment ? `liseng-runtime-dev-${Date.now()}` : 'liseng-runtime-v1';
...
self.addEventListener('push', (event) => {
  if (event.data) {
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'LisEng', options)
    );
  }
});
