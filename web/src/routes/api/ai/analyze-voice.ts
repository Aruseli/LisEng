...
export const Route = createFileRoute('/api/ai/analyze-voice')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const {
            ...
            level = 'A2',
            userId,
          } = (await request.json()) as AnalyzeVoiceRequest

          if (!transcription || typeof transcription !== 'string') {
            return jsonError('transcription is required', 400)
          }

          // Получаем язык инструкций пользователя
          let instructionLanguage = 'ru' // По умолчанию русский
          const effectiveUserId = userId || who.userId
          try {
            instructionLanguage = await getUserInstructionLanguage(
              getAdminClient(),
              effectiveUserId,
            )
          } catch (error) {
...
`
          ...
          const normalizedFeedback: VoiceFeedback = {
            pronunciation: {
              ...
              issues: Array.isArray(feedback.pronunciation?.issues)
                ? feedback.pronunciation.issues
                : [],
            },
            vocabulary: {
              targetWordsUsed: Array.isArray(feedback.vocabulary?.targetWordsUsed)
                ? feedback.vocabulary.targetWordsUsed
                : [],
              missingWords: Array.isArray(feedback.vocabulary?.missingWords)
                ? feedback.vocabulary.missingWords
                : [],
              feedback: feedback.vocabulary?.feedback || 'Использование словаря соответствует уровню.',
              suggestions: Array.isArray(feedback.vocabulary?.suggestions)
            ...
            overall: {
              ...
              suggestions: Array.isArray(feedback.overall?.suggestions)
                ? feedback.overall.suggestions
                : [],
            },
          }

          return Response.json(normalizedFeedback)
        } catch (error: any) {
