...
async function runIntegrationTest() {
  ...
  try {
    ...
    const {
      ...
      ScheduleService
    } = await import('./lib/lesson-snapshots/index.js');
    ...
    try {
      ...
      console.log('⚠️ ProgressInsightsService вернул ошибку (возможно, нет данных):', error.message);
    ...
    try {
      ...
      console.log('⚠️ Ошибка при получении snapshot insights:', error.message);
    }
    ...
    try {
      ...
      const snapshotId = await lessonSnapshotService.createSnapshot(snapshotData);
      console.log('✅ Слепок урока создан, ID:', snapshotId);
    } catch (error) {
      console.log('⚠️ Ошибка при создании слепка:', error.message);
      console.log('⏭️ Продолжаем тест...');
    }
    ...
    try {
      ...
      const testId = await shuHaRiService.generateWeeklyTest(testUserId, new Date(), 'comprehensive', true);
      ...
      console.log('ℹ️ Shu-Ha-Ri тест не создан (возможно, нет прогресса):', error.message);
      console.log('⏭️ Продолжаем тест...');
    }
    ...
    try {
      ...
      console.log('⚠️ Ошибка при создании расписания:', error.message);
      console.log('⏭️ Продолжаем тест...');
    }
